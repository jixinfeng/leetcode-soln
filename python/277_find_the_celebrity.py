...
ask questions like: "Hi, A. Do you know B?" to get information about whether A knows B. You need to find out the
...
class Solution:
    def findCelebrity(self, n: int) -> int:
        ...
        if all([knows(j, candidate) for j in range(n) if candidate != j]) and not any(
                [knows(candidate, j) for j in range(n) if candidate != j]):
            return candidate
