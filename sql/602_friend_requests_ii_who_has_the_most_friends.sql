...
Table request_accepted holds the data of friend acceptance, while requester_id
and accepter_id both are the id of a person.
...
| 3  | 3   |
Note:
It is guaranteed there is only 1 people having the most friends.  The friend
request could only been accepted once, which mean there is no multiple records
...
*/
SELECT DISTINCT p1 AS id, COUNT(p2) AS num
FROM (
    SELECT requester_id AS p1, accepter_id AS p2
    FROM request_accepted
    UNION ALL
    SELECT accepter_id AS p1, requester_id AS p2
    FROM request_accepted
) AS friends
GROUP BY id
ORDER BY num DESC
